...
def random_noise(x, y, max_eps, mode='global'):
    if mode == 'global':
        rand_noise = (2*torch.rand(x.size()).to(conf.device)-1) * max_eps / 255.0
    elif mode == 'background':
        mask  = get_mask(x, y)
        rand_noise = (2*torch.rand(x.size()).to(conf.device)-1) * max_eps * mask / 255.0
    return torch.clamp(x + rand_noise, 0, 1) 

def ens_mi_fgsm(x, y, model_frcnn, model_yolo, model_ssd, loss_yolo, loss_ssd, max_eps, num_iter, momentum):
    mask  = get_mask(x, y)
    eps = max_eps * mask / 255.0
    x_min = torch.clamp(x - max_eps, 0, 1)
    x_max = torch.clamp(x + max_eps, 0, 1)
    ...
    for i in range(num_iter):
        ...
        # compute grad to l_fn of img
        x_adv = x
        noise_ensemble = grad_ensemble(model_frcnn, model_yolo, model_ssd, x_adv, y, loss_yolo, loss_ssd, conf.w_frcnn, conf.w_yolo, conf.w_ssd, 'all')
        noise = noise_ensemble /  torch.mean(torch.abs(noise_ensemble), (1, 2, 3), keepdims=True)
        grad = momentum * grad + noise
        x = x + alpha * torch.sign(grad)
        x = clip_by_value(x, x_min, x_max)
    return x
