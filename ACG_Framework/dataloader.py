...
class Dataset(Dataset):
    def __init__(self, class2idx_path, input_img_path, clean_img_path, label_path, input_shape, num_classes, anchors_yolo, anchors_mask, anchors_ssd, epoch_length, \
                        train, special_aug_ratio = 0.7, overlap_threshold=0.5):
        ...
        self.img_filename_list = os.listdir(self.input_img_path)
        self.length             = len(self.img_filename_list)
    ...
    def __getitem__(self, index):
        ...
        image, box, location, input_CAP_filename, idxs = self.get_random_data(self.img_filename_list[index], self.input_shape, random = self.train)

        image_clean = self.get_random_data_clean(os.path.join(self.clean_img_path, input_CAP_filename), self.input_shape, random = self.train)


        image       = np.transpose(preprocess_input(np.array(image, dtype=np.float32)), (2, 0, 1))
        image_clean = np.transpose(preprocess_input(np.array(image_clean, dtype=np.float32)), (2, 0, 1))
        ...
        return image, boxes_frcnn, labels_frcnn, boxes_yolo, labels_yolo, boxes_ssd, location, input_CAP_filename, idxs, box, image_clean
    ...
    def get_random_data(self, img_filename, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        #------------------------------#
        #   读取图像并转换成RGB图像
        #------------------------------#
        image   = Image.open(os.path.join(self.input_img_path, img_filename))
        ...
        #   获得预测框
        #------------------------------#
        label_filename = img_filename.replace('png', 'json')
    
        with open(os.path.join(self.label_path, label_filename), 'r', encoding='utf-8') as f:
            json_dic = json.load(f)
        char_names = []
        box_list = []
        # print(json_dic['shapes'])
        for s in json_dic['shapes']:
            char_names.append(s['label'])
        ...
        for name in char_names:
            ...
            idxs.append(int(self.class2idx[name]))
        ...
        if not random:
            ...
            if len(box)>0:
                ...
                # combined_lists = list(zip(box, idxs))
                # np.random.shuffle(combined_lists)
                # box, idxs = zip(*combined_lists)

                indices = np.arange(len(box))
                np.random.shuffle(indices)
                box = box[indices]
                idxs = [idxs[i] for i in indices]
            ...
            #     print(idxs)
            # print('----------')
            return image_data, box, location, img_filename, idxs
    ...
    def get_random_data_clean(self, clean_img_path, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        #------------------------------#
        #   读取图像并转换成RGB图像
        #------------------------------#
        # print('s1ghhh', clean_img_path)
...
def dataset_collate(batch):
    ...
    boxes = []
    images_clean = []
    for img, box_frcnn, label_frcnn, box_yolo, label_yolo, box_ssd, location, path, idx, box, image_clean in batch:
        images.append(img)
    ...
    images_clean  = torch.from_numpy(np.array(images_clean)).type(torch.FloatTensor)
    ...
    return images, targets, locations, paths, idxs, boxes, images_clean
